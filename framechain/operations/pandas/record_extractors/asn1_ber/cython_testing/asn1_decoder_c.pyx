import framechain.operations.pandas.record_extractors.asn1_ber.exceptions
from framechain import exceptions
import logging

log = logging.getLogger(__name__)

cdef class Asn1Decoder(object):
    cdef bytes asn_data
    cdef long asn_index
    cdef dict header_trailer_lengths, record_schemas, decode_cache
    cdef int recordtype_depth, current_depth
    cdef public object current_record_schema

    def __init__(self, list record_schemas, dict header_trailer_lengths):
        """
        ...
        :param str recordtype_field_name: Name of field to store recordtype name in
        """
        ...
        # log.debug('Recordtype depth:  {}, schemas: {}'.format(self.recordtype_depth, self.record_schemas))
    ...
    cpdef void skip_until_asn_block(self):
        """
    ...
    cpdef object decode_node(self, object parent_node, long start_pos=0):
        """
        ...
        """
        cdef int tag_len, tag_type, tag_number, value_len, byte, len_byte
        cdef object node

        if start_pos == 0:
            start_pos = self.asn_index

        # Check cache to see if record type has already been decoded
        for tag_len in [2,3,4]:
            if self.asn_data[start_pos:start_pos+tag_len] in self.decode_cache:
                tag_type, tag_number, value_len = self.decode_cache[self.asn_data[start_pos:start_pos+tag_len]]
                break
        # Decode new tag
        else:
            ...
            tag_type = self.asn_data[start_pos] & 0x20
            # ----------GET TAG NUMBER---------------------#
            ...
            # Store in cache
            self.decode_cache[self.asn_data[start_pos:start_pos + tag_len]] = (tag_type, tag_number, value_len)
        # --------------CONSTRUCT NODE------------------#
        node = ASN1Node(
            start_pos=start_pos,
            tag_len=tag_len,
    ...
    cpdef dict build_asn_record(self, object root_node):
        """
    ...
    cdef void traverse_asn(self, object node, object current_record=None):
        ...
        if current_record is not None:
            ...
            elif (node.tag_type == 0) and (node.id in self.current_record_schema.target_fields):
                field_name = self.current_record_schema.target_fields[node.id]
                # log.debug('Found field: {}'.format(field_name))
                current_record[field_name] = self.get_node_value(node)
    ...
    cdef bint is_node_last_child(self, object node):
        """
    ...
    cdef bytes get_node_value(self, object node):
        """
        ...
        if node.end_pos != 0:
            ...
            raise framechain.operations.pandas.record_extractors.asn1_ber.exceptions.ASNDecodeError('Cannot get data of node with no end position')
    ...
    cdef object next_node(self, object node):
        """
    ...
    cdef object first_child_node(self, object node):
        """
        ...
        if node.tag_type:
            ...
            raise framechain.operations.pandas.record_extractors.asn1_ber.exceptions.ASNDecodeError('Cant get child node of primitive node')


cdef class ASN1Node(object):
    """
    Object representing ASN1node, mainly containing attributes describing the position of the node in ASNS1 data
    """
    cdef public long start_pos, value_pos, end_pos
    cdef public long long id
    cdef public int tag_type
    cdef public object parent

    #__slots__ = ['start_pos','value_pos','end_pos', 'type', 'parent', 'id']

    def __init__(self, long start_pos, int tag_len, int value_len, int tag_type, object parent_node, int tag_number):
        """
        :param int start_pos: index of start of ASN1 node in ASN1 data file
        :param int tag_len: length of tag data (number of bytes encoding tag number, type, length)
        :param int value_len: length of node data/content
        :param int tag_type: (0 - Primitive, 1 - Constructed)
        :param ASN1Node parent_node: parent node object
        :param int tag_number: ASN1 ID of element
        """
        self.start_pos = start_pos
        self.value_pos = start_pos + tag_len
        self.end_pos = 0 if value_len == 0 else (start_pos + tag_len + value_len)
        self.tag_type = tag_type
        self.parent = parent_node
        # Calculate absolute ID of node (from root element)
        self.id = (tag_number if parent_node is None else (parent_node.id<<8) + tag_number) + 1

    def __str__(self):
        return 'ASN1 Node {}: {}:{}:{}'.format(self.id, self.start_pos, self.value_pos, self.end_pos)


class ASN1RecordSchema(object):
    """
    Class which represents definition for an ASN1 record schema.
    Includes:
    - Record type name
    ...
    """
    def __init__(self, recordtype_name, recordtype_tag, field_tags):
        """

        :param str recordtype_name: Name of record type
        :param str recordtype_tag: ASN1 tag ID corresponding to record type
        :param tuple field_tags: list of field names and corresponding ASN1 tag IDs of fields to be extracted (excluding recordtype_tag)
        """
        ...
        self.target_fields = {self.convert_asn_tag_to_unique_integer('-'.join([recordtype_tag, field_tag])): field_name for field_name, field_tag in field_tags}
